...
CURL_COMMON_ARGS="--retry 5 --retry-delay 1 --retry-connrefused --connect-timeout 10 --max-time 120"

curl_fetch() {
  local url="$1"
  if [ -z "$url" ]; then
    echo "curl_fetch missing URL"
    return 2
  fi
  # shellcheck disable=SC2086
  curl -fsSL $CURL_COMMON_ARGS "$url"
}
...
if [ -n "${COMMIT_ID}" ]; then
  ...
  commit_payload=$(curl_fetch "https://update.code.visualstudio.com/api/commits/stable/server-$DETECTED_PLATFORM-$ARCH-web" || true)
  HASH=$(printf "%s" "$commit_payload" | cut -d '"' -f 2)
fi
if [ -z "$HASH" ]; then
  echo "Failed to resolve VS Code Web commit id. Provide COMMIT_ID or check network connectivity."
  exit 1
fi
printf "$${BOLD}VS Code Web commit id version $HASH.\n"

output=$(curl_fetch "https://vscode.download.prss.microsoft.com/dbazure/download/stable/$HASH/vscode-server-$DETECTED_PLATFORM-$ARCH-web.tar.gz" | tar -xz -C "${INSTALL_PREFIX}" --strip-components 1)
